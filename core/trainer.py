...
def train_eval(model, ims, real_input_flag, itr):
    cost = model.train(ims, real_input_flag)
    ims_rev = np.flip(ims, axis=1).copy()
    cost += model.train_eval(ims_rev, real_input_flag)
    cost = cost / 2
    return cost
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        for i in range(output_length):
            ...
            for b in range(configs.batch_size):
                score, _ = compare_ssim(np.squeeze(pred_frm[b]), np.squeeze(real_frm[b]), full=True, multichannel=False)
...
def test_eval(model, test_input_handle, configs, itr):
    print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'test...')
    test_input_handle.begin(do_shuffle=False)
    #res_path = os.path.join(configs.gen_frm_dir, str(itr))
    #os.mkdir(res_path)
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        test_input_handle.next()
    img_mse = np.array(img_mse) / (batch_id * configs.batch_size)
    ssim = np.array(ssim, dtype=np.float32) / (configs.batch_size * batch_id)
    psnr = np.array(psnr, dtype=np.float32) / batch_id
    lp = np.array(lp, dtype=np.float32) / batch_id
    return img_mse, ssim, psnr, lp
